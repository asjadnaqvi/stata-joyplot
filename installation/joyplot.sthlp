...
{* 28May2023}{...}
{hi:help joyplot}{...}
{right:{browse "https://github.com/asjadnaqvi/stata-joyplot":joyplot v1.62 (GitHub)}}

{hline}

{title:joyplot}: A Stata package for joy or ridgeline plots. 
...
{cmd:joyplot} {it:y} {it:[x]} {ifin}, {cmd:by}({it:variable}) 
                {cmd:[} {cmd:overlap}({it:num}) {cmdab:bwid:th}({it:num}) {cmd:palette}({it:str}) {cmd:alpha}({it:num}) {cmdab:off:set}({it:num}) {cmd:lines} 
                   {cmd:droplow} {cmdab:norm:alize}({it:local} | {it:global}) {cmd:rescale} {cmdab:off:set}({it:num}) {cmdab:laboff:set}({it:num})
                   ...
                   {cmd:note}({it:str}) {cmd:scheme}({it:str}) {cmd:name}({it:str}) 
                {cmd:]}
{p 4 4 2}
...
{p2coldent : {opt bwid:th(value)}}A higher bandwidth value will result in higher smoothing. The default value is {opt bwid(0.1)}.
...
{p2coldent : {opt overlap(value)}}A higher value increases the overlap, and the height of the joyplots. The default value is {opt overlap(6)}, and the minimum allowed value is 1.{p_end}
...
{p2coldent : {opt alpha(value)}}Alpha is used to change the transparency of the area fills. Default value is {opt alpha(80)} for 80% transparency.{p_end}

{p2coldent : {opt lines}}Draw colored lines instead of area fills. This option is much faster since area fills are computationally intensive. The option {opt lcolor()} does not work here. 
Instead define the line colors using the {cmd:palette()} option. The option {opt lwidth()} is allowed.{p_end}
...
{p2coldent : {opt lc:olor(string)}}The outline color of the area fills. Default is {opt lc(white)}.{p_end}

{p2coldent : {opt lw:idth(value)}}The outline width of the area fills. Default is {opt lw(0.15)}.{p_end}

{p2coldent : {opt yl:ine}}Shows base reference lines for each {opt by()} category.{p_end}

{p2coldent : {opt ylc:olor(string)}}The color of the y-axis grids lines. Default is {opt ylc(black)}.{p_end}

{p2coldent : {opt ylw:idth(value)}}The width of the y-axis grids lines. Default is {opt ylw(0.04)}.{p_end}

{p2coldent : {opt ylp:attern(string)}}The pattern of the y-axis grids lines. Default is {opt ylp(solid)}.{p_end}

{p2coldent : {opt ylabpos:ition(string)}}The position of the y-axis labels takes on the values {it:left} or {it:right}. The default orientation is {opt ylabpos(left)}.{p_end}

{p2coldent : {opt laboff:set(value)}}This option is used for offseting the labels on the y-axis. Default is {opt laboffset(0)} for zero pixels.
Or {opt offset(-20)} will move the labels left by 20 pixels if the label has a {it:left} orientation.{p_end}

{p2coldent : {opt off:set(value)}}This option is used for extending the range of the x-axis to show labels. Default is {opt offset(15)} or 15%. {p_end}
...
See {browse "https://github.com/asjadnaqvi/stata-joyplot":GitHub}.
